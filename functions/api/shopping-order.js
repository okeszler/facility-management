...
// sort_order = Position im übergebenen Array; checked-Gruppierung entscheidet
// weiterhin, welche Einträge oben/unten stehen (siehe buildShoppingList).
export async function onRequestPost(context) {
  ...
  try {
    ...
    const writes = ids.map((id, i) =>
      db.prepare('UPDATE shopping_items SET sort_order = ?1 WHERE id = ?2').bind(i, id)
    );
