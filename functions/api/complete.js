...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (task.one_off) {
      ...
      await logAction(db, id, task.name, isEarly ? 'vorgezogen erledigt (einmalig)' : 'erledigt (einmalig)', user);
    } else {
      ...
      await logAction(db, id, task.name, isEarly ? 'vorgezogen erledigt' : 'erledigt', user);
    }
